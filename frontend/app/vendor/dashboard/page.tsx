...
type VendorStats = {
  ...
  recentLeads?: Array<{ id: string; userName: string; userEmail: string; serviceRequirement: string; status: string; createdAt: string }>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_20px_50px_rgba(15,23,42,0.06)] md:p-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Leads</p>
            <h2 className="mt-2 text-2xl font-semibold text-slate-950">Recent consultation activity</h2>
          </div>

          <div className="mt-5 space-y-3">
            {(stats?.recentLeads || []).slice(0, 4).map((lead) => (
              <article key={lead.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-slate-950">{lead.userName}</p>
                    <p className="mt-1 text-sm text-slate-600">{lead.serviceRequirement}</p>
                    <p className="mt-1 text-xs text-slate-500">{new Date(lead.createdAt).toLocaleString()}</p>
                  </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_20px_50px_rgba(15,23,42,0.06)] md:p-6">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-slate-950">Latest alerts</h2>
          </div>

          <div className="mt-5 space-y-3">
            {(stats?.recentNotifications || []).slice(0, 4).map((notification) => (
              <article key={notification.id} className={`rounded-2xl border px-4 py-3 ${notification.isRead ? "border-slate-200 bg-slate-50" : "border-amber-200 bg-amber-50"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
