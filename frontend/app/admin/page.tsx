...
export default function AdminLoginPage() {
  ...
  return (
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10 sm:px-6 lg:px-8">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.12),transparent_32%),radial-gradient(circle_at_bottom_right,rgba(31,143,77,0.08),transparent_38%)]" />
      ...
      <div className="relative grid w-full max-w-5xl overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_32px_70px_rgba(15,23,42,0.14)] lg:grid-cols-[1.1fr_1fr]">
        <div className="hidden border-r border-slate-200/80 bg-slate-950 px-10 py-12 text-white lg:flex lg:flex-col lg:justify-between">
          <div>
            <BrandMark
              ...
              taglineClassName="text-emerald-200/80"
            />
            <p className="mt-8 text-xs font-semibold uppercase tracking-[0.3em] text-emerald-300">Admin Access</p>
        ...
        <div className="px-6 py-8 sm:px-9 sm:py-10 lg:px-10 lg:py-12">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-600">Admin Login</p>
          ...
          <form className="mt-7 space-y-5" onSubmit={handleLogin}>
            <label className="block text-sm font-semibold text-slate-700">
              ...
              <input
                ...
                required
                className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-200/70"
              />
            </label>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-lg border border-emerald-300/70 bg-emerald-400 px-5 py-3 text-sm font-semibold uppercase tracking-[0.18em] text-slate-950 transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-55"
            >
              {loading ? "Signing In..." : "Sign In"}
            </button>
          </form>

          <div className="mt-7 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs text-emerald-900">
