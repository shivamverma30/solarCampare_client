  ...
  const card = (
    <div className="rounded-[28px] bg-[rgba(255,255,255,0.14)] backdrop-blur-[20px] border border-white/20 p-4 shadow-[0_10px_30px_rgba(2,6,23,0.12)] ring-1 ring-white/10 transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_14px_40px_rgba(2,6,23,0.14)] sm:p-5">
      <div className="flex flex-col gap-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-white/90">TIME LEFT TO GET BENEFIT OF SCHEME</p>
          <h3 className="mt-2 text-lg font-extrabold text-white sm:text-xl">Scheme closes on 31 March 2027</h3>
          <p className="mt-2 text-sm leading-6 text-white/70">Hurry — apply before the scheme deadline to secure benefits and subsidies.</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {values.map((item) => (
            <div key={item.label} className="rounded-2xl bg-[rgba(255,255,255,0.12)] backdrop-blur border border-white/20 px-3 py-3 text-center shadow-[0_6px_18px_rgba(2,6,23,0.08)] ring-1 ring-white/10">
                  ...
                  <div className="mt-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/70">{item.label}</div>
            </div>
          ))}
        </div>
      </div>
