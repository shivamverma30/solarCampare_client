...
function LanguageSwitcher({ locale, setLocale, isDark }: { locale: string; setLocale: (l: string) => void; isDark: boolean }) {
  ...
  const cls = isDark
    ...
    : "inline-flex h-10 w-10 items-center justify-center rounded-full border border-sky-200 bg-white text-sky-500 shadow-sm";
...
export default function Navbar() {
  ...
  const headerBase = overlay
    ...
    : "bg-white !text-black shadow-[0_16px_40px_rgba(15,23,42,0.14)] border-b border-slate-200";

  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-colors duration-200 ${headerBase}`}>
      <div className={`relative mx-auto flex max-w-[1600px] items-center gap-6 px-6 py-4 lg:px-8`}> 
        ...
        <div className="relative z-10 flex w-full items-center">
          <div className="flex-1 min-w-0">
            <BrandMark href="/" compact titleClassName={overlay ? "text-amber-50" : "!text-black"} taglineClassName={overlay ? "text-amber-100/80" : "!text-black/70"} />
          </div>
          ...
          <nav className="hidden lg:flex flex-1 justify-center gap-8">
            {navLinks.map((l) => {
              ...
              return (
                <Link key={l.href} href={l.href} className={`group text-[15px] font-medium uppercase tracking-[0.05em] ${base} px-1 py-2`}>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden lg:flex items-center gap-3">
              ...
              <Link href="/login" className={overlay ? "rounded-full border border-white/16 bg-white/12 px-5 py-2.5 text-sm text-amber-50" : "rounded-full border border-slate-300 bg-white px-5 py-2.5 text-sm !text-black shadow-[0_8px_20px_rgba(15,23,42,0.08)]"}>{t("buttons.login")}</Link>
              <Link href="/calculator" className="hidden md:inline-block rounded-full bg-[#ffd23f] px-5 py-2.5 text-sm font-semibold !text-black shadow-[0_8px_20px_rgba(15,23,42,0.12)]">{t("buttons.getProposal")}</Link>
            </div>
        ...
        <div className={`absolute left-0 right-0 top-full z-40 lg:hidden transition-all duration-300 ${open ? "block" : "hidden"}`}>
          <div className={`mx-auto max-w-[1600px] rounded-b-xl bg-white shadow-md border-t border-slate-200`}> 
            <nav className="grid gap-1 p-3">
              ...
              <div className="mt-2 flex gap-2 px-3 pb-3">
                ...
                <Link href="/calculator" className="flex-1 rounded-full bg-[#ffd23f] px-4 py-2 text-sm !text-black">{t("buttons.getProposal")}</Link>
              </div>
            </nav>
          </div>
        </div>
